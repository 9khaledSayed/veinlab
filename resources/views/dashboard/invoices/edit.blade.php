...
<div class="row">
    <div class="col-lg-12">
        ...
        <div class="kt-portlet">
            ...
            @endif
            <!--begin::Form-->
            <form class="kt-form kt-form--label-right" id="waiting-lab-form" action="{{route('dashboard.invoices.update', $invoice)}}" method="post">
                @csrf
                @method('put')
                <div class="kt-portlet__body">
                            ...
                            <div class="form-group">
                                ...
                                <div class="kt-radio-inline">
                                    <label class="kt-radio">
                                        <input type="radio" name="promo_code" value="0" checked  > {{__('No Codes')}}
                                        <span></span>
                                    </label>
                                    <label class="kt-radio">
                                        <input type="radio" name="promo_code" value="1"  @if($invoice->promo_code_id) checked @endif> {{__('Have A Code')}}
                                        <span></span>
                                    </label>
                                </div>
                            </div>
                                ...
                                <div class="kt-radio-inline">
                                    <label class="kt-radio">
                                        <input type="radio"
                                               data-action="#package"
                                               name="transfer"
                                               value="{{config('enums.transfer.individual')}}" checked > {{__('Not Found')}}
                                        <span></span>
                                    </label>
                                    <label class="kt-radio">
                                        <input type="radio"
                                               ...
                                               name="transfer"
                                               value="{{config('enums.transfer.contract')}}" @if($invoice->company_id) checked @endif
                                        > {{__('Contract')}}
                                        <span></span>
                                    </label>
                                    <label class="kt-radio">
                                        <input type="radio"
                                               ...
                                               name="transfer"
                                               value="{{config('enums.transfer.hospital')}}" @if($invoice->hospital_id) checked @endif
                                        > {{__('Hospital')}}
                                        <span></span>
                                    </label>
                                    <label class="kt-radio">
                                        <input type="radio"
                                               data-action="#doctors"
                                               name="transfer"
                                               value="{{config('enums.transfer.doctor')}}" @if($invoice->doctor_id) checked @endif
                                        > {{__('Doctor')}}
                                        <span></span>
                                    </label>
                                    <label class="kt-radio">
                                        <input type="radio"
                                               data-action="#sectors"
                                               name="transfer"
                                               value="{{config('enums.transfer.sector')}}" @if($invoice->sector_id) checked @endif
                                        > {{__('Section')}}
                                        <span></span>
                                    </label>
                                </div>
                    ...
                    <div class="form-group row">
                        <div class="col-lg-4">
                            ...
                            <select class="form-control @error('patient_id') is-invalid @enderror kt-selectpicker"
                                ...
                                @forelse($patients as $patient)
                                    <option
                                        value="{{$patient->id}}"
                                        @if($patient->id == $invoice->patient_id) selected @endif
                                    >{{$patient->name}}</option>
                                @empty
                        ...
                        <input type="hidden" id="analyses_ids" value='@json(unserialize($invoice->main_analysis) ?? [])'>
                        <div class="col-lg-4">
                        ...
                        <div class="col-lg-4" id="package">
                            ...
                            <select class="form-control @error('package_id')is-invalid @enderror kt-selectpicker"
                                ...
                                @forelse($packages as $package)
                                    <option
                                        value="{{$package->id}}"
                                        {{ (in_array($package->id, unserialize($invoice->packages) ?? [])) ? 'selected':'' }}
                        ...
                        <div class="col-lg-4">
                            ...
                            <select class="form-control @error('company_id')is-invalid @enderror kt-selectpicker"
                                ...
                                @forelse($companies as $company)
                                    <option value="{{$company->id}}" @if($company->id == $invoice->company_id) selected @endif>{{$company->name}}</option>
                                @empty
                                    <option disabled>{{__('There is no Companies')}}</option>
                                @endforelse
                            </select>
                        </div>
                        <input type="hidden" id="invoice_category_id" value="{{ $invoice->category_id }}">
                        <div class="col-lg-4">
                        ...
                        <div class="col-lg-4" id="hospitals" style="display: none">
                            ...
                            <select
                                ...
                                @forelse($hospitals as $hospital)
                                    <option
                                        value="{{$hospital->id}}"
                                        @if($hospital->id == $invoice->hospital_id) selected @endif
                        ...
                        <div class="col-lg-4" id="doctors" style="display:none;">
                            ...
                            <select class="form-control @error('doctor_id')is-invalid @enderror kt-selectpicker"
                                ...
                                @forelse($doctors as $doctor)
                                    <option
                                        value="{{$doctor->id}}"
                                        @if($doctor->id == $invoice->doctor_id) selected @endif
                                    >{{$doctor->name}}</option>
                                @empty
                        ...
                        <div class="col-lg-4" id="promo_codes" style="display: none">
                            ...
                            <select class="form-control @error('code_id') is-invalid @enderror kt-selectpicker"
                                ...
                                @forelse($promo_codes as $code)
                                    <option
                                        value="{{$code->id}}"
                                        @if($code->id == $invoice->promo_code_id) selected @endif
                        ...
                        <div class="col-lg-4" id="sectors" style="display: none">
                            ...
                            <select class="form-control @error('sector_id') is-invalid @enderror kt-selectpicker"
                                ...
                                @forelse($sectors as $sector)
                                    <option
                                        value="{{$sector->id}}"
                                        @if($sector->id == $invoice->sector_id) selected @endif
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-lg-3">
                            ...
                            <input type="text"
                                   ...
                                   name="rest"
                                   value="{{$invoice->amount_paid - $invoice->total_price}}"
                                   readonly
                        ...
                        <div class="col-lg-3">
                            ...
                            <select name="pay_method"
                                    ...
                                    title="{{__('Choose')}}">
                                <option value="{{config('enums.payMethod.cash')}}" @if($invoice->pay_method == config('enums.payMethod.cash')) selected @endif>{{__('Cash')}}</option>
                                <option value="{{config('enums.payMethod.credit')}}" @if($invoice->pay_method == config('enums.payMethod.credit')) selected @endif>{{__('Credit')}}</option>
                                <option value="{{config('enums.payMethod.overdue')}}" @if($invoice->pay_method == config('enums.payMethod.overdue')) selected @endif>{{__('Overdue')}}</option>
                            </select>
                        </div>
                    </div>
                    <div class="form-group row">
                ...
                <div class="form-group row">
                    <div class="col-lg-4 mx-auto text-center">
                        ...
                        <div>
                            <h5 id="total_amount">{{ $invoice->total_price }}</h5>
                        </div>
