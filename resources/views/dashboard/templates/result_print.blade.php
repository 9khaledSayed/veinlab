...
<body>
    ...
    <div class="page-header" style="text-align: center">
        <img class="logo mt-2" src="{{asset(Setting::get('logo_path'))}}" width="" height="95%" alt="">
    </div>

    <div class="page-footer">
        <div class="row">
            <div class="col-3">
                <h3 class="text-bold" style="color:#3F4493">مختبرات</h3>
                <h2 class="text-danger text-bold">نبض الوقاية الطبية</h2>
            </div>
            <div class="col-3">
                <div class="h6 contact-info">
                    <i class="fa-solid fa-phone footer-icon"></i>
                    +966596411010 / +966536521010
                </div>
                <div class="h6 contact-info">
                    <i class="fa-solid fa-envelope footer-icon"></i>
                    Preventionpulse2030@gmail.com
                </div>
            </div>
            <div class="col-3">
                <img src="{{asset($invoice->branch->report_signature)}}" width="150">
                <h5 class="text-center" style="margin:0;">{{ $invoice->doctor }}</h4>
                    <h6 style="margin:0;color:darkgray;font-size:12px;text-align:center;">Consultant Clinical
                        Scientist<br> Immunologist</h6>
            </div>
        ...
        <tbody>
            <tr>
                <td>
                    ...
                    <!--*** CONTENT GOES HERE ***-->
                    <div class="page" style="margin-top: 15px;  page-break-after:auto">
                        <div class="row">
                            ...
                            <div class="col-6 d-flex flex-column justify-content-center px-5">
                                <div class="row d-flex">
                                    ...
                                    <span class="user-info">Identity NO: {{ $invoice->patient->id_no }}</span>
                                </div>
                                <div class="row d-flex">
                                ...
                                <div class="row d-flex">
                                    <span class="user-info">Result Time: {{ $invoice->created_at->format('Y-m-d h:i
                                        A')}}</span>
                                </div>
                                <div class="row d-flex">
                                    <span class="user-info">Medical Center: Nabd Elwqaya Laboratories</span>
                                    <span class="user-info">License No: {{ $invoice->branch->license_no }}</span>
                                </div>
                            </div>
                        </div>
                        @foreach($waitingLabsByDivision as $divisionId => $waitingLabs)
                            <h2 class="text-primary ml-5">{{ \App\Division::find($divisionId) ? \App\Division::find($divisionId)["name"]: 'Not Classified' }}</h2>
                            @foreach ($waitingLabs as $waitingLab)
                            <div class="content" style="margin-top: 10px">
                                ...
                                <table class="table-borderless w-75 mx-auto">
                                    ...
                                    <tbody>
                                        ...
                                        @foreach($waitingLab->results->groupBy('classification') as $classification =>
                                        $results)
                                        @if($classification != "-")
                                        <tr>
                                        ...
                                        <tr style="{{ $loop->index }}">
                                            ...
                                            <td>
                                                {!! $result->sub_analysis->normal($invoice->patient->gender) ?
                                                $result->sub_analysis->normal($invoice->patient->gender) : '-' !!}
                                            </td>
